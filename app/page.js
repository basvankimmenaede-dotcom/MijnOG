...
function TrainingEditor({ profile, teams, competitions = [], profiles, memberships, event, onClose, onSaved }) {
  ...
  async function save() {
    ...
    const payload = { team_id:Number(selectedTeamIds[0]), type:form.type, competition_id:form.type==='game'?Number(competitionId):null, status:form.type==='game'?(event?.status||'scheduled'):'scheduled', audience_mode:form.type==='training'?audienceMode:'all', title:form.title.trim()||activityLabel, description:form.description.trim()||null, start_at:combineDateTime(form.date,form.start), end_at:form.end?combineDateTime(form.date,form.end):null, meet_at:form.meet?combineDateTime(form.date,form.meet):null, location_name:form.location_name.trim()||null, location_address:form.location_address.trim()||null, updated_at:new Date().toISOString() }
    try {
...
function coreStatsForPlayer(personId, team, attendance=[], gameAttendance=[], trainingEvents=[], calendarEvents=[], gameStats=[], measurements=[]) {
  ...
  return {avg,obp,ab:sums.ab||0,h:sums.h||0,bb:sums.bb||0,rbi:hasPartial?null:(sums.rbi||0),sb:hasPartial?null:(sums.sb||0),hasPartial,exit:latest('exit_velocity'),home1:latest('home_to_first'),attendance:att}
}
function statRate(v){return v==null?'—':Number(v).toFixed(3).replace(/^0/,'')}
function statPercent(v){return v==null?'—':`${Math.round(Number(v)*100)}%`}
...
function SwingAnalyzerModal({ session, profile, accessLevel, team, players = [], profiles = [], memberships = [], onClose }) {
  ...
  function stopBulkAnalysis(){
    ...
    setBulkProgress(prev=>({...prev,label:'Stoppen na huidige stap…'}))
  }
  function retryBulkItem(key){
    if(bulkBusy)return
    setBulkQueue(prev=>prev.map(q=>q.key===key?{...q,status:'ready',error:null,score:null}:q))
  }
  async function analyzeBulkQueue(){
    ...
    for(const item of bulkQueue){
      ...
      try{
        const quality=await withTimeout(checkSwingVideoQuality(item.file),30000,'Kwaliteitscontrole duurde langer dan 30 seconden. Het videobestand kon niet goed worden uitgelezen.')
        if(bulkCancelRef.current){setBulkQueue(prev=>prev.map(q=>q.key===item.key?{...q,status:'ready',error:'Analyse gestopt door gebruiker.'}:q));break}
        if(quality?.status==='red')throw new Error(quality?.detail?`Kwaliteitscontrole afgekeurd: ${quality.detail}`:'Kwaliteitscontrole afgekeurd: onvoldoende betrouwbaar voor AI-analyse.')
        setBulkQueue(prev=>prev.map(q=>q.key===item.key?{...q,status:'analyzing',quality}:q))
        const result=await withTimeout(analyzeSwingVideo(item.file,(progress)=>setBulkProgress({current,total,label:`${item.file.name} · ${progress?.label||'Analyseren…'}`})),90000,'AI-analyse duurde langer dan 90 seconden. Waarschijnlijk kon een videoframe of het AI-model niet verder worden verwerkt.')
        if(bulkCancelRef.current){setBulkQueue(prev=>prev.map(q=>q.key===item.key?{...q,status:'ready',error:'Analyse gestopt door gebruiker.'}:q));break}
        const {overall,focus}=resultScoreInfo(result)
        const meta=item.meta||{}
        const payload={player_id:selectedPlayer.id,coach_id:session.user.id,team_id:team?.id||null,recorded_at:meta.recordedAt||new Date(item.file.lastModified||Date.now()).toISOString(),overall_score:overall,metrics:result.metrics,focus,exit_velocity:null,coach_note:null,metric_confidence:result.confidences||{},analysis_meta:{...(result.meta||{}),observations:result.observations||{},video_quality:quality||null,source_filename:item.file.name,source_account:meta.sourceAccount||null,source_swing_id:meta.sourceSwingId||null,bulk_session_id:bulkSessionId},analysis_version:'pose-ai-v1.2',source:meta.source||'video_upload',source_swing_id:meta.sourceSwingId||null,source_filename:item.file.name,session_id:bulkSessionId}
        const {data,error:insertError}=await withTimeout(supabase.from('swing_analyses').insert(payload).select().single(),20000,'Opslaan in MijnOG duurde te lang. Controleer de internetverbinding en probeer deze swing opnieuw.')
        if(insertError){
          if(insertError.code==='23505'){setBulkQueue(prev=>prev.map(q=>q.key===item.key?{...q,status:'duplicate',error:null}:q));continue}
          throw new Error(`Opslaan mislukt: ${insertError.message||'onbekende databasefout'}`)
        }
        newAnalyses.push(data)
        setBulkQueue(prev=>prev.map(q=>q.key===item.key?{...q,status:'done',score:Math.round(data.overall_score),error:null}:q))
      }catch(err){
        const reason=err?.message||'Analyse mislukt zonder foutmelding.'
        setBulkQueue(prev=>prev.map(q=>q.key===item.key?{...q,status:'error',error:reason}:q))
      }
    }
    if(newAnalyses.length)setAnalyses(prev=>[...newAnalyses.reverse(),...prev])
    const stopped=bulkCancelRef.current
    setBulkProgress({current:Math.min(current,total),total,label:stopped?'Bulk-analyse gestopt':'Bulk-analyse afgerond'})
    setBulkBusy(false)
    bulkCancelRef.current=false
  }
    ...
    <div className="swing-body">
      ...
      {busy && view==='home' ? <div className="swing-loading">Analyses laden…</div> : <>
        ...
        {view==='new' && selectedPlayer && <>
          ...
          <section className="swing-capture-card"><SwingCameraCapture videoUrl={videoUrl} onVideo={pickVideo} onVideos={pickVideos} videoQuality={videoQuality} qualityBusy={qualityBusy}/>{videoFile&&<small className="swing-file-name">{videoFile.name} · {(videoFile.size/1024/1024).toFixed(1)} MB · wordt niet opgeslagen</small>}{bulkQueue.length>1&&<div className="swing-bulk"><div className="swing-bulk-head"><span><strong>{bulkQueue.length} swings geselecteerd</strong><small>Alle video’s worden gekoppeld aan {personName(selectedPlayer)}.</small></span><b>{bulkQueue.filter(i=>i.status==='done').length}/{bulkQueue.filter(i=>i.status!=='duplicate').length}</b></div><div className="swing-bulk-list">{bulkQueue.map(item=><article key={item.key} className={`status-${item.status}`}><span><strong>{item.meta?.recordedAt?new Date(item.meta.recordedAt).toLocaleTimeString('nl-NL',{hour:'2-digit',minute:'2-digit',second:'2-digit'}):item.file.name}</strong><small>{item.meta?.isBlast?'Blast Softball':item.file.name}</small>{item.error&&<em className="swing-bulk-error">{item.error}</em>}</span><div className="swing-bulk-state"><b>{item.status==='duplicate'?'Bestaat al':item.status==='checking'?'Controleren':item.status==='analyzing'?'Analyseren':item.status==='done'?`${item.score}/100`:item.status==='error'?'Mislukt':'Klaar'}</b>{item.status==='error'&&!bulkBusy&&<button type="button" onClick={()=>retryBulkItem(item.key)}>Opnieuw</button>}</div></article>)}</div>{bulkBusy&&<div className="swing-ai-progress swing-bulk-progress"><div><i style={{width:`${bulkProgress.total?Math.round((bulkProgress.current/bulkProgress.total)*100):0}%`}}/></div><strong>{bulkProgress.label||'Analyseren…'}</strong><small>{bulkProgress.current}/{bulkProgress.total}</small></div>}<div className="swing-bulk-actions"><button className="swing-primary" disabled={bulkBusy||!bulkQueue.some(i=>i.status==='ready'||i.status==='error')} onClick={analyzeBulkQueue}><Icon name="swing"/>{bulkBusy?'Swings analyseren…':'Alle nieuwe swings analyseren & opslaan'}</button>{bulkBusy&&<button type="button" className="swing-secondary swing-stop-bulk" onClick={stopBulkAnalysis}>Stop analyse</button>}</div><p className="swing-help swing-bulk-note">Blast-bestandsnamen worden alleen gebruikt voor datum/tijd en duplicaatcontrole. De gekozen MijnOG-speelster blijft altijd leidend.</p></div>}</section>
...
function playerTeamOptions(rows = [], teams = []) {
  const seen=new Map()
  rows.forEach(row=>{
    const id=Number(row.team_id)
    if(!Number.isFinite(id)||seen.has(id)) return
    const team=teams.find(t=>Number(t.id)===id)
    seen.set(id,row.teams?.name||team?.name||`Team ${id}`)
  })
  return [...seen.entries()].map(([id,name])=>({id:String(id),name})).sort((a,b)=>a.name.localeCompare(b.name,'nl',{numeric:true}))
}

function statTeamName(row, teams = []) {
  const team=teams.find(t=>Number(t.id)===Number(row?.team_id))
  return row?.teams?.name || team?.name || (row?.team_id!=null ? `Team ${row.team_id}` : 'Onbekend team')
}

function PlayerGameHistoryModal({ person, rows = [], competitions = [], teams = [], context = '', onClose }) {
  return <SettingsModal title="Wedstrijdstats" onClose={onClose}>
    <div className="player-game-history-modal">
      <div className="player-game-history-intro"><div><p className="eyebrow orange">PER WEDSTRIJD</p><h3>{person ? personName(person) : 'Mijn wedstrijden'}</h3></div>{context&&<small>{context}</small>}</div>
      {rows.length ? <div className="stats-game-history player-game-history-list">{[...rows].sort((a,b)=>new Date(b.game_date)-new Date(a.game_date)).map(row=>{
        const avg=Number(row.ab||0)>0?Number(row.h||0)/Number(row.ab):null
        return <article key={row.id||`${row.game_key}-${row.profile_id}`}>
          <div className="stats-game-history-head"><span>{new Date(`${row.game_date}T12:00:00`).toLocaleDateString('nl-NL',{day:'numeric',month:'long',year:'numeric'})}</span><strong>{row.opponent||'Wedstrijd'}</strong></div>
          <div className="player-game-history-context"><span>{statTeamName(row,teams)}</span><span>{competitionNameForStat(row,competitions)}</span></div>
          <div className="stats-game-history-values player-game-history-values"><span><b>{Number(row.ab||0)}</b> AB</span><span><b>{Number(row.h||0)}</b> H</span><span><b>{Number(row.bb||0)}</b> BB</span><span><b>{statRate(avg)}</b> AVG</span>{!row.partial_batting&&<span><b>{Number(row.rbi||0)}</b> RBI</span>}{!row.partial_batting&&<span><b>{Number(row.sb||0)}</b> SB</span>}</div>
        </article>
      })}</div> : <div className="stats-empty-state">Geen wedstrijden binnen dit filter.</div>}
    </div>
  </SettingsModal>
}
...
function Stats({ profile, teams = [], competitions = [], attendance = [], gameAttendance = [], trainingEvents = [], calendarEvents = [], gameStats = [], measurements = [], pitchingStats = [], playerCards = [], playerCardMetrics = [], memberships = [], onPrepareProfile, onSaved }) {
  ...
  const teamGames=teamFilter==='total'?seasonGames:seasonGames.filter(row=>String(row.team_id)===String(teamFilter))
  const competitionOptions=playerCompetitionOptions(teamGames,competitions)
  const ownGames=competitionFilter==='total'?teamGames:teamGames.filter(row=>competitionKeyForStat(row,competitions)===competitionFilter)
  ...
  const selectedStatsTeam=teamFilter==='total'?null:teams.find(t=>String(t.id)===String(teamFilter))||null
  const att=attendanceStatsForPerson(profile?.id,selectedStatsTeam,attendance,gameAttendance,trainingEvents,calendarEvents,year)
  ...
  return <section className="stats-page stats-growth-page">
    ...
    <div className="stats-context-filters">
      <label>Seizoen<select value={seasonYear} onChange={e=>{setSeasonYear(e.target.value);setTeamFilter('total');setCompetitionFilter('total')}}>{yearOptions.length?yearOptions.map(y=><option key={y} value={y}>{y}</option>):<option value={year}>{year}</option>}</select></label>
      <label>Team<select value={teamFilter} onChange={e=>{setTeamFilter(e.target.value);setCompetitionFilter('total')}}><option value="total">Alle teams</option>{teamOptions.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></label>
...
function PlayerProfileModal({ person, team, teams = [], competitions = [], viewerProfile, viewerMembership, attendance = [], gameAttendance = [], trainingEvents = [], calendarEvents = [], gameStats = [], measurements = [], pitchingStats = [], playerCards = [], playerCardMetrics = [], onChanged, onClose }) {
  ...
  const playerTeamGames=profileTeam==='total'?playerSeasonGames:playerSeasonGames.filter(row=>String(row.team_id)===String(profileTeam))
  const profileCompetitionOptions=playerCompetitionOptions(playerTeamGames,competitions)
  const playerFilteredGames=profileCompetition==='total'?playerTeamGames:playerTeamGames.filter(row=>competitionKeyForStat(row,competitions)===profileCompetition)
  const performance=canSeePlayerStats?battingRates(playerFilteredGames):null
  const selectedPitching=pitchingStats.filter(row=>row.profile_id===person?.id&&statSeasonYear(row)===profileYear&&(profileTeam==='total'||String(row.team_id)===String(profileTeam))&&(profileCompetition==='total'||competitionKeyForStat(row,competitions)===profileCompetition))
  const pitching=canSeePlayerStats?pitchingStatsForPlayer(person?.id,null,selectedPitching):null
  const selectedProfileTeam=profileTeam==='total'?null:teams.find(t=>String(t.id)===String(profileTeam))||null
  const stats = canSeeAttendance ? attendanceStatsForPerson(person?.id, selectedProfileTeam, attendance, gameAttendance, trainingEvents, calendarEvents, profileYear) : null
...
function AdminPanel({ session, onMessage, onChanged }) {
  ...
  const [staffForm, setStaffForm] = useState({ profile_id:'', staff_role:'video_performance', label:'', scope:'club', team_id:'' })
  ...
  async function loadAdminData() {
    ...
    const [seasonResult, teamResult, profileResult, memberResult, locationResult, pushResult, staffResult] = await Promise.all([
      ...
      supabase.from('member_task_invitations').select('*,teams(id,name)').order('created_at',{ascending:false})
    ])
  ...
  async function addClubStaff(e) {
    ...
    if(staffForm.scope==='team'&&!staffForm.team_id)return onMessage('Kies eerst een team.')
    const teamId=staffForm.scope==='team'?Number(staffForm.team_id):null,existing=clubStaffRows.find(row=>row.profile_id===staffForm.profile_id&&row.task_role===staffForm.staff_role&&Number(row.team_id||0)===Number(teamId||0));setBusy(true);const payload={profile_id:staffForm.profile_id,task_role:staffForm.staff_role,team_id:teamId,note:staffForm.label.trim()||null,status:'invited',invited_by:session.user.id,responded_at:null};const {error}=existing?await supabase.from('member_task_invitations').update(payload).eq('id',existing.id):await supabase.from('member_task_invitations').insert(payload);setBusy(false)
    if(error)return onMessage(`Taakuitnodiging versturen mislukt: ${error.message}`)
    setStaffForm({profile_id:'',staff_role:'video_performance',label:'',scope:'club',team_id:''});onMessage('Taakuitnodiging verstuurd ✓');await loadAdminData()
  }
  ...
  async function toggleClubStaff(row){if(!window.confirm('Deze taakuitnodiging verwijderen?'))return;setBusy(true);const {error}=await supabase.from('member_task_invitations').delete().eq('id',row.id);setBusy(false);if(error)onMessage(error.message);else await loadAdminData()}
  ...
  const visibleClubStaffRows=clubStaffRows.filter(row=>{const person=profiles.find(p=>p.id===row.profile_id);return personMatchesSearch(person,staffSearch,`${staffRoleLabel(row.task_role)} ${row.teams?.name||''} ${row.note||''}`)})
  ...
  return (
    <>
      <button className="admin-launcher" onClick={() => setOpen(true)}>
        ...
        <span className="settings-copy"><strong>Clubbeheer</strong><small>Leden, teams, taken, functies en locaties</small></span>
        ...
        <div className="admin-modal-backdrop" role="presentation" onMouseDown={e => { if (e.target === e.currentTarget) closeAdmin() }}>
          <section className="club-admin-modal" role="dialog" aria-modal="true" aria-label="Clubbeheer">
            <header className="sheet-header">
              ...
              <div><p className="eyebrow orange">BEHEERDER</p><h2>{view === 'menu' ? 'Clubbeheer' : view === 'seasons' ? 'Seizoenen' : view === 'teams' ? 'Teams' : view === 'members' ? 'Team toewijzen' : view === 'directory' ? 'Ledenlijst' : view === 'locations' ? 'Locaties' : view === 'staff' ? 'Taken & functies' : view === 'roles' ? 'Rollen & rechten' : 'Lid uitnodigen'}</h2></div>
              ...
              <div className="sheet-body">
                {view === 'menu' && (
                  <div className="admin-menu-list">
                    ...
                    <AdminMenuItem icon="person" title="Taken & functies" subtitle="Nodig leden uit voor een club- of teamtaak" onClick={() => goTo('staff')} />
                ...
                {view === 'staff' && (
                  <div className="admin-block">
                    <form className="sheet-form form-stack" onSubmit={addClubStaff}>
                      <div><p className="eyebrow orange">TAAKUITNODIGING</p><h3>Nodig een lid uit</h3><p className="muted">Iedereen is clublid. Hier wijs je alleen een concrete functie toe.</p></div><PersonSearch value={staffSearch} onChange={setStaffSearch} placeholder="Zoek lid" /><label>Lid<select value={staffForm.profile_id} onChange={e=>setStaffForm({...staffForm,profile_id:e.target.value})} required><option value="">Kies clublid</option>{visibleStaffProfiles.map(person=><option key={person.id} value={person.id}>{personName(person)}</option>)}</select></label>
                      <div className="admin-form-grid"><label>Taak<select value={staffForm.staff_role} onChange={e=>setStaffForm({...staffForm,staff_role:e.target.value})}>{['head_coach','assistant_coach','pitching_trainer','catching_trainer','team_manager','physio','mental_coach','strength_conditioning','video_performance','scorer','other'].map(role=><option key={role} value={role}>{staffRoleLabel(role)}</option>)}</select></label><label>Niveau<select value={staffForm.scope} onChange={e=>setStaffForm({...staffForm,scope:e.target.value,team_id:e.target.value==='club'?'':staffForm.team_id})}><option value="club">Hele club</option><option value="team">Specifiek team</option></select></label></div>{staffForm.scope==='team'&&<label>Team<select value={staffForm.team_id} onChange={e=>setStaffForm({...staffForm,team_id:e.target.value})} required><option value="">Kies team</option>{teams.filter(team=>team.is_active).map(team=><option key={team.id} value={team.id}>{team.name}</option>)}</select></label>}<label>Toelichting<input value={staffForm.label} onChange={e=>setStaffForm({...staffForm,label:e.target.value})} placeholder="Optionele uitleg bij de taak"/></label>
                      <button className="primary" disabled={busy}>Uitnodiging versturen</button>
                    </form>
                    <div className="admin-list">{visibleClubStaffRows.map(row=>{const person=profiles.find(p=>p.id===row.profile_id);return <article className="admin-row" key={row.id}><div><strong>{personName(person)}</strong><small>{staffRoleLabel(row.task_role)} · {row.teams?.name||'Hele club'}{row.note?` · ${row.note}`:''}</small></div><span className={`task-status ${row.status}`}>{row.status==='accepted'?'Geaccepteerd':row.status==='declined'?'Afgewezen':'Wacht op reactie'}</span><button className="mini-action" onClick={()=>toggleClubStaff(row)}>Verwijderen</button></article>})}{!clubStaffRows.length&&<div className="admin-empty">Nog geen taakuitnodigingen.</div>}</div>
                  </div>
                )}
...
function More({ session, profile, teams, competitions = [], calendar, attendance = [], gameAttendance = [], trainingEvents = [], calendarEvents = [], memberships = [], gameStats = [], measurements = [], pitchingStats = [], playerCards = [], playerCardMetrics = [], onSaved, onMessage }) {
  ...
  useEffect(()=>{if(settingsView==='tasks')supabase.from('member_task_invitations').select('*,teams(id,name)').eq('profile_id',session.user.id).order('created_at',{ascending:false}).then(({data})=>setTaskInvitations(data||[]))},[settingsView])
  ...
  async function respondTask(invitation,status){setTaskBusy(true);const {error}=await supabase.rpc('respond_member_task_invitation',{invitation_id:invitation.id,new_status:status});if(error)onMessage(error.message);else{setTaskInvitations(rows=>rows.map(row=>row.id===invitation.id?{...row,status}:row));onMessage(status==='accepted'?'Taak geaccepteerd ✓':'Taak afgewezen.')}setTaskBusy(false)}
...
function normalizeTrainingEvent(row, eventTeamLinks = [], participantLinks = []) {
  ...
  return {
    id: row.id,
    uid: row.external_source === 'foys' && row.external_uid ? row.external_uid : `${row.type || 'training'}-${row.id}`,
    type: row.type === 'game' ? 'game' : 'training',
    source: row.external_source === 'foys' ? 'foys' : 'supabase',
    teamId: row.team_id,
    teamIds,
