...
export async function POST(request){
  try{
    ...
    for(const row of subs||[]){try{await webpush.sendNotification({endpoint:row.endpoint,keys:{p256dh:row.p256dh,auth:row.auth}},JSON.stringify({title:'Invallerverzoek van een coach',body:`${req.event_title} · gezocht: ${req.position}`,url:'/?tab=Coach',tag:`coach-request-${req.id}`}));sent++}catch(e){if([404,410].includes(e?.statusCode))await service.from('push_subscriptions').delete().eq('id',row.id)}}
