...
function waitEvent(el,event,timeoutMs=10000){return new Promise((resolve,reject)=>{let timer=null;const ok=()=>{cleanup();resolve()};const bad=()=>{cleanup();reject(new Error('Video kon niet worden gelezen. Het bestand of de videocodec wordt mogelijk niet ondersteund.'))};const timeout=()=>{cleanup();reject(new Error(event==='loadedmetadata'?'Video-informatie kon niet binnen 10 seconden worden gelezen. Controleer het bestand of probeer de video opnieuw te exporteren.':'De video reageerde niet tijdens het uitlezen van frames. Probeer de video opnieuw te uploaden of exporteren.'))};const cleanup=()=>{if(timer)clearTimeout(timer);el.removeEventListener(event,ok);el.removeEventListener('error',bad)};el.addEventListener(event,ok,{once:true});el.addEventListener('error',bad,{once:true});timer=setTimeout(timeout,timeoutMs)})}
async function seek(video,t){if(Math.abs(video.currentTime-t)<.002)return;video.currentTime=t;await waitEvent(video,'seeked',8000)}
